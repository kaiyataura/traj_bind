...
def train(data_dir: str, mode: Literal['teacher', 'student'] = 'teacher', 
    ...
    with tqdm(range(start_epoch, num_epochs + 1), initial=start_epoch, total=num_epochs, desc="Epochs", leave=False) as pbar1:
        for epoch in pbar1:
            ...
            class SimpleScaler:
                def __init__(self, scale_factor=1024.0):
                ...
                def step(self, optimizer):
                    for group in optimizer.param_groups:
                        for param in group['params']:
                            if param.grad is not None:
                                param.grad.data.mul_(1.0 / self.scale_factor)
                    optimizer.step()
                def update(self):
                    pass
            ...
            scaler = SimpleScaler(1024.0) if device == 'mps' else None
