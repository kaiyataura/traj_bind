    ...
    with tqdm(dataloader, desc=desc, leave=False) as pbar:
        with torch.no_grad():
            for i, batch in enumerate(pbar):
                batch = {k:x.to(device) if isinstance(x, torch.Tensor) else x for k, x in batch.items()}
                with torch.autocast(device_type='mps', dtype=torch.float16, enabled=device == 'mps'):
                    out_dict = model(**batch, inference=False)
...
def train(data_dir: str, mode: Literal['teacher', 'student'] = 'teacher', 
          device: str = 'mps', num_epochs: int = 100, batch_size: int = 32, accum_steps: int = 1, num_frames: int = 16, valid_frac: float = 0.1,
    ...
    if mode == 'teacher':
        ...
        loss_fn = JointLoss([(InteractionLoss(), 1.0), (AffinityLoss(accum_steps=accum_steps), 1.0)])
        dataset = TrajectoryDataset(data_dir, num_frames=num_frames)
        
    elif mode == 'student':
        if base_ckpt is None: raise ValueError("Argument `base_ckpt` must be defined for student mode.")
        model = StudentModel().to(device)
        loss_fn = JointLoss([(DistillationLoss(), 1.0), (AffinityLoss(accum_steps=accum_steps), 1.0)])
            ...
            with tqdm(train_dataloader, desc="Batches", leave=False) as pbar2:
                ...
                for i, batch in enumerate(pbar2):
                    ...
                    if device == 'mps':
                        ...
                        if profile_time: torch.mps.synchronize()
...
if __name__ == '__main__':
    train(
        ...
        batch_size=1, accum_steps=8, num_frames=16, lr_readout=1e-3, lr_scaffold=5e-4,
        param_every=1, eval_every=1, checkpoint_every=10,
        out_dir='checkpoints/run17'
    )