...
class SpatialAttention(nn.Module):
    ...
    def forward(self, node_embed: torch.Tensor, edge_embed: torch.Tensor, dir: torch.Tensor, mask: torch.Tensor, frame: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        edge_in = torch.concat([node_embed.unsqueeze(2).expand(-1, -1, N, -1), 
                                node_embed.unsqueeze(1).expand(-1, N, -1, -1), 
                                edge_embed], dim=-1) # [B, N, N, D*3]
        edge_embed = edge_embed + self.edge_update(edge_in) # [B, N, N, D]
...
class TeacherModel(nn.Module):
    def __init__(self, hidden_dim: int = 128, **kwargs):
        ...
        nn.init.zeros_(self.enthalpy_mlp[-1].weight)
        nn.init.zeros_(self.enthalpy_mlp[-1].bias)
    ...
    def _process_frame(self, coords_t: torch.Tensor, residues: torch.Tensor, props: torch.Tensor, masks: torch.Tensor, core_masks: torch.Tensor):
        node_embed, edge_embed = self.spatial_encoder(coords_t, residues, props, masks) # [B, N, D], [B, N, N, D]
        
        interactions = self.interaction_head(edge_embed) # [B, N, N, 4]
        ...
        return interactions, graph_embed, node_embed

    def _process_chunk(self, coords_chunk: torch.Tensor, residues: torch.Tensor, props: torch.Tensor, masks: torch.Tensor, core_masks: torch.Tensor):
        """Process a temporal chunk of frames [B, C, N, 4, 3] through the spatial encoder."""
        B, C, N, _, _ = coords_chunk.shape
        all_interactions, all_graph_embeds, all_node_embeds = [], [], []
        for c in range(C):
            interactions_t, graph_embed_t, node_embed_t = self._process_frame(
                coords_chunk[:, c], residues, props, masks, core_masks
            )
            all_interactions.append(interactions_t)
            all_graph_embeds.append(graph_embed_t)
            all_node_embeds.append(node_embed_t)
        return torch.stack(all_interactions, dim=1), torch.stack(all_graph_embeds, dim=1), torch.stack(all_node_embeds, dim=1)

    def forward(self, coords: torch.Tensor, residues: torch.Tensor, props: torch.Tensor, masks: torch.Tensor, core_masks: torch.Tensor, inference: bool = False, temporal_chunk_size: int = 1, **kwargs):
        # coords: [B, T, N, 4, 3]
        # residues: [B, N]
        # props: [B, N, 5]
        # masks: [B, N]
        ...
        if self.training:
            coords = coords + 0.1 * torch.randn_like(coords) * masks.view(B, 1, N, 1, 1)

        all_interactions = []
        all_graph_embeds = []
        all_node_embeds = []

        for t_start in range(0, T, temporal_chunk_size):
            chunk = coords[:, t_start:t_start + temporal_chunk_size]
            if self.training:
                inter_c, graph_c, node_c = torch.utils.checkpoint.checkpoint(
                    self._process_chunk,
                    chunk, residues, props, masks, core_masks,
                    use_reentrant=False
                )
            else:
                inter_c, graph_c, node_c = self._process_chunk(
                    chunk, residues, props, masks, core_masks
                )
            all_interactions.append(inter_c)
            all_graph_embeds.append(graph_c)
            all_node_embeds.append(node_c)

        interactions = torch.cat(all_interactions, dim=1)  # [B, T, N, N, 4]
        graph_embeds = torch.cat(all_graph_embeds, dim=1)  # [B, T, D]
        node_embeds = torch.cat(all_node_embeds, dim=1)    # [B, T, N, D]
        
        enthalpy_embeds = self.enthalpy_mlp(graph_embeds) # [B, T, D]
        energies = self.energy_head(enthalpy_embeds).squeeze(2) # [B, T]
        beta = torch.exp(self.log_beta)
        log_probs = F.log_softmax(-beta * energies, dim=1) # [B, T]
        ...
        # Compute Boltzmann-weighted edge_embed incrementally to avoid
        # materializing the full [B, T, N, N, D] tensor in memory.
        # Only needed for distillation target generation (offline, eval mode).
        # Computing it during training forces autograd to save T copies of
        # the [B, N, N, D] tensor for the backward pass of `probs`, causing OOM.
        if not self.training:
            D = node_embeds.shape[-1]
            edge_embed = torch.zeros(B, N, N, D, device=coords.device) # [B, N, N, D]
            for t in range(T):
                with torch.no_grad():
                    _, edge_embed_t = self.spatial_encoder(coords[:, t], residues, props, masks) 
                edge_embed = edge_embed + edge_embed_t * probs[:, t].view(B, 1, 1, 1)
        else:
            edge_embed = None
...
class AffinityLoss(nn.Module):
    def __init__(self, affinity_weight: float = 0.1, rank_weight: float = 1.0, accum_steps: int = 1):
        super().__init__()
    ...
    def _rank_loss(self, preds, targets, mask_preds=None, mask_targets=None):
        if mask_preds is None: mask_preds, mask_targets = preds, targets
        pred_diff = preds.unsqueeze(1) - mask_preds.unsqueeze(0) # [N, M, 1]
        label_diff = targets.unsqueeze(1) - mask_targets.unsqueeze(0) # [N, M]
        mask = (label_diff > 0).float().unsqueeze(-1) # [N, M, 1]
        return (-F.logsigmoid(pred_diff) * mask).sum(), mask.sum()

    def forward(self, affinities, affinity_targets, **kwargs):
        affinity_loss = ((affinities - affinity_targets.unsqueeze(1)) ** 2).mean() # []

        if self.training and self.accum_steps > 1 and len(self.queue) > 0:
            past_preds, past_targets = map(lambda x: torch.cat(x, dim=0), zip(*self.queue))
            all_preds = torch.cat([affinities, past_preds], dim=0)
            all_targets = torch.cat([affinity_targets, past_targets], dim=0)
            
            loss_fwd, count_fwd = self._rank_loss(affinities, affinity_targets, all_preds, all_targets)
            loss_bwd, count_bwd = self._rank_loss(all_preds, all_targets, affinities, affinity_targets)
            rank_loss = (loss_fwd + loss_bwd) / max((count_fwd + count_bwd), 1e-8)
        else:
            loss, count = self._rank_loss(affinities, affinity_targets)
            rank_loss = loss / max(count, 1e-8) if affinities.shape[0] > 1 else torch.tensor(0.0, device=affinities.device)

        if self.training and self.accum_steps > 1:
            self.queue.append((affinities.detach(), affinity_targets.detach()))
            if len(self.queue) >= self.accum_steps: self.queue.pop(0)
