...
def train(data_dir: str, mode: Literal['pretrain', 'teacher', 'student'] = 'pretrain', 
          ...
          lr_readout: float = 1e-3, lr_scaffold: float = 1e-3, stop_lr: float | None = 1e-5,
...
if __name__ == '__main__':
    ...
    #     batch_size=2, accum_size=8, chunk_size=2, lr_readout=1e-3, lr_scaffold=1e-3, patience=10,
    ...
    #     batch_size=8, accum_size=1, chunk_size=2, num_frames=4, lr_readout=1e-3, lr_scaffold=5e-5, patience=5,
    #     param_every=1, eval_every=1, checkpoint_every=5,
    #     base_ckpt='checkpoints/run58_pretrain/best.pt',
    #     out_dir='checkpoints/run61_58_warmup',
    # )

    train(
        data_dir='data/dynarepo/dataset.h5', mode='teacher',
        device='mps', dtype=torch.float32, num_epochs=1000, valid_frac=0.1,
        batch_size=8, accum_size=1, chunk_size=1, lr_readout=1e-3, lr_scaffold=1e-3, patience=5,
        param_every=1, eval_every=1, checkpoint_every=5,
        base_ckpt='checkpoints/run63_58_train/epoch_0.pt',
        out_dir='checkpoints/run63_58_train',
    )