...
if __name__ == '__main__':
    ...
    train(
        ...
        device='mps', dtype=torch.float32, num_epochs=50, valid_frac=0.1,
        batch_size=8, accum_size=1, chunk_size=1, num_frames=1, 
        lr_head=1e-3, lr_gate=1e-3, lr_scaffold=1e-3, warmup_epochs=2, patience=5,
        param_every=1, eval_every=1, checkpoint_every=5,
        out_dir='checkpoints/run01_pretrain'
    )
    ...
    #     lr_head=1e-3, lr_gate=5e-4, lr_scaffold=1e-4, warmup_epochs=2, patience=5, weight_decay=1e-3,
    #     param_every=1, eval_every=1, checkpoint_every=5,
    #     base_ckpt='checkpoints/run01_pretrain/best.pt',
    #     out_dir='checkpoints/run02_warmup'
    # )

    # train(
    #     data_dir='data/dynarepo/dataset.h5', mode='teacher',
    #     device='mps', dtype=torch.float32, num_epochs=200, valid_frac=0.1,
    #     batch_size=4, accum_size=2, chunk_size=1, num_frames=16, 
    #     lr_head=1e-3, lr_gate=1e-4, lr_scaffold=1e-4, warmup_epochs=2, patience=10, weight_decay=1e-3,
    #     param_every=1, eval_every=1, checkpoint_every=5,
    #     base_ckpt='checkpoints/run02_warmup/best.pt',
    #     out_dir='checkpoints/run03_teacher'
    # )
    
    # STEP 4: STUDENT DISTILLATION (T=1)
    # Freeze the full Teacher, pre-compute the joint embeddings (embeds.pt), and 
    # train the static Student model to hallucinate the ensemble thermodynamics.
    # train(
    #     data_dir='data/dynarepo/dataset.h5', mode='student',
    #     device='mps', dtype=torch.float32, num_epochs=300, valid_frac=0.1,
    #     batch_size=8, accum_size=1, chunk_size=1, num_frames=16, 
    #     lr_head=1e-3, lr_gate=5e-4, lr_scaffold=5e-4, warmup_epochs=2, patience=10, weight_decay=1e-3,
    #     param_every=1, eval_every=1, checkpoint_every=5,
    #     base_ckpt='checkpoints/run03_teacher/best.pt',
    #     out_dir='checkpoints/run04_student'
    # )