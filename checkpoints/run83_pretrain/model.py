...
class DistanceEmbedding(nn.Module):
    ...
    def forward(self, coords: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        d = dists.flatten(-2).unsqueeze(-1) # [B, N, N, 16, 1]
        cutoff = 0.5 * (torch.cos(torch.clamp(d, max=self.r_max) * (math.pi / self.r_max)) + 1.0) * (d <= self.r_max)
        rbfs = (torch.exp(((d - self.centers) ** 2) * -self.gamma) * cutoff).flatten(-2) # [B, N, N, Sd]
...
class StudentModel(nn.Module):
    ...
    def forward(self, coords: torch.Tensor, residues: torch.Tensor, props: torch.Tensor, valid_masks: torch.Tensor, core_masks: torch.Tensor, ligand_masks: torch.Tensor, mode: Literal['inference', 'train', 'distill', 'embed'] = 'inference', **kwargs):
        ...
        pair_embed = torch.concat([edge_embed, geom_embed], dim=-1) # [B, N, N, 2 * D]
        enthalpy_embed = self.enthalpy_mlp(pair_embed) * self.enthalpy_gate(pair_embed).sigmoid() # [B, N, N, D]
        entropy_embed = self.entropy_mlp(node_embed) * self.entropy_gate(node_embed).sigmoid() # [B, N, D]
        edge_mask = core_masks[:,None,:] & core_masks[:,:,None] & (dist <= 15.0) & (ligand_masks[:,None,:] != ligand_masks[:,:,None]) # [B, N, N]
        enthalpy = (self.enthalpy_head(enthalpy_embed) * edge_mask[:,:,:,None]).sum(dim=(1,2,3)) # [B]
        entropy = (self.entropy_head(entropy_embed) * core_masks[:,:,None]).sum(dim=(1,2)) # [B]
        affinity = (enthalpy + entropy + self.affinity_bias.squeeze(-1)).squeeze(-1) # [B]

        if mode == 'inference': return affinity, enthalpy, entropy
