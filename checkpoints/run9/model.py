...
class CheckpointMLP(nn.Module):
    ...
    def forward(self, *args):
        if self.training and args[0].requires_grad:
            return ckpt(self.mlp, *args, use_reentrant=False)
...
class SpatialAttention(nn.Module):
    def __init__(self, hidden_dim: int, num_heads: int = 4, dropout: float = 0.1):
        ...
        nn.init.zeros_(self.bias_mlp.mlp[-1].weight)
        ...
        self.edge_update = CheckpointMLP(
        ...
        nn.init.zeros_(self.edge_update.mlp[-1].weight)
        nn.init.zeros_(self.edge_update.mlp[-1].bias)
...
class TeacherModel(nn.Module):
    def __init__(self, hidden_dim: int = 128, **kwargs):
        ...
        self.gate_mlp = CheckpointMLP(
            nn.LayerNorm(hidden_dim),
            nn.Linear(hidden_dim, hidden_dim // 2),
            nn.SiLU(),
            nn.Linear(hidden_dim // 2, 1)
        )
        nn.init.zeros_(self.gate_mlp.mlp[-1].weight)
        nn.init.zeros_(self.gate_mlp.mlp[-1].bias)
        ...
        self.interaction_head = CheckpointMLP(
            nn.LayerNorm(hidden_dim),
            nn.Linear(hidden_dim, hidden_dim),
            nn.SiLU(),
