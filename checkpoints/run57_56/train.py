...
def train(data_dir: str, mode: Literal['pretrain', 'teacher', 'student'] = 'pretrain', 
    ...
    if mode == 'pretrain':
        ...
        model = TeacherModel(**kwargs).to(device).to(dtype)
        if base_ckpt is not None: model.load_state_dict(torch.load(base_ckpt, map_location='cpu', weights_only=False)['model_state_dict'])
...
if __name__ == '__main__':
    train(
        ...
        batch_size=8, accum_size=2, lr_readout=1e-3, lr_scaffold=1e-3, patience=5,
        param_every=1, eval_every=1, checkpoint_every=10,
        base_ckpt='checkpoints/run56_pretrain/best.pt',
        out_dir='checkpoints/run57_56',
    )
    # train(
    #     data_dir='data/dynarepo/dataset.h5', mode='pretrain',
    #     device='mps', dtype=torch.float32, num_epochs=1000, valid_frac=0.1,
    #     batch_size=2, accum_size=16, chunk_size=32, lr_readout=1e-3, lr_scaffold=1e-3, patience=10,
    #     param_every=1, eval_every=1, checkpoint_every=10,
    #     out_dir='checkpoints/run56_pretrain',
    # )