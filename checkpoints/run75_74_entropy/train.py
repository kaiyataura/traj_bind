...
def train(data_dir: str, mode: Literal['pretrain', 'teacher', 'student'] = 'pretrain', 
          ...
          lr_head: float = 1e-4, lr_gate: float = 5e-3, lr_scaffold: float = 1e-3, warmup_epochs: int = 1, stop_lr: float | None = 1e-5,
    ...
    if mode == 'pretrain':
        ...
        for p in model.entropy_mlp.parameters(): p.requires_grad = True
        for p in model.entropy_head.parameters(): p.requires_grad = True
        for p in model.entropy_gate.parameters(): p.requires_grad = True
...
if __name__ == '__main__':
    ...
    train(
        data_dir='data/dynarepo/dataset.h5', mode='teacher',
        device='mps', dtype=torch.float32, num_epochs=1000, valid_frac=0.1,
        batch_size=8, accum_size=1, chunk_size=1, num_frames=4, lr_head=1e-3, lr_gate=1e-3, lr_scaffold=1e-4, warmup_epochs=1, patience=10,
        param_every=1, eval_every=1, checkpoint_every=5,
        base_ckpt='checkpoints/run75_74_entropy/epoch_0.pt',
        out_dir='checkpoints/run75_74_entropy',
    )