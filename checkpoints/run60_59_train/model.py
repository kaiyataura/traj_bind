...
class TeacherModel(nn.Module):
    ...
    def forward(self, coords: torch.Tensor, residues: torch.Tensor, props: torch.Tensor, masks: torch.Tensor, core_masks: torch.Tensor, mode: Literal['inference', 'pretrain', 'train', 'embed'] = 'inference', chunk_size: int = 1, **kwargs):
        # coords: [B, T, N, 4, 3]
        # residues: [B, N]
        # props: [B, N, 5]
        # masks: [B, N]
        ...
        masks_flat = masks.repeat_interleave(T, 0)
        core_masks_flat = core_masks.repeat_interleave(T, 0)

        def process_chunk(coords_chunk, residues_chunk, props_chunk, masks_chunk, core_masks_chunk):
            node_embeds_chunk, edge_embeds_chunk, geom_embeds_chunk = self.spatial_encoder(coords_chunk, residues_chunk, props_chunk, masks_chunk)
            
            interactions_chunk = self.interaction_head(torch.concat([edge_embeds_chunk, geom_embeds_chunk], dim=-1)) if mode == 'pretrain' or mode == 'train' else None
            if mode == 'pretrain': return {'interactions': interactions_chunk}
            
            edge_masks_chunk = core_masks_chunk[:,None,:,None] & core_masks_chunk[:,:,None,None]
            enthalpy_embeds_chunk = self.enthalpy_mlp(edge_embeds_chunk)
            enthalpies_chunk = (self.enthalpy_head(enthalpy_embeds_chunk) * self.enthalpy_gate(edge_embeds_chunk).sigmoid() * edge_masks_chunk).sum((-2,-3))
            entropy_embeds_chunk = self.entropy_mlp(node_embeds_chunk)
            return {'node_embeds': node_embeds_chunk, 'enthalpies': enthalpies_chunk, 'entropy_embeds': entropy_embeds_chunk, **({'interactions': interactions_chunk} if mode == 'train' else {}), **({'enthalpy_embeds': enthalpy_embeds_chunk} if mode == 'embed' else {})}
        ...
        for start in range(0, B * T, chunk_size):
            ...
            masks_chunk = masks_flat[start:end]
            core_masks_chunk = core_masks_flat[start:end]

            if self.training: chunk_out = checkpoint(process_chunk, coords_chunk, residues_chunk, props_chunk, masks_chunk, core_masks_chunk, use_reentrant=False)
            else: chunk_out = process_chunk(coords_chunk, residues_chunk, props_chunk, masks_chunk, core_masks_chunk)
        ...
        entropy_embeds = out['entropy_embeds'] # [B, T, N, D]
...
class StudentModel(nn.Module):
    ...
    def forward(self, coords: torch.Tensor, residues: torch.Tensor, props: torch.Tensor, masks: torch.Tensor, core_masks: torch.Tensor, mode: Literal['inference', 'train'] = 'inference', **kwargs):
        ...
        if self.training:
            coords = coords + self.noise * torch.randn_like(coords) * masks.view(B, N, 1, 1) # [B, N, 4, 3]
            
        node_embed, edge_embed = self.spatial_encoder(coords, residues, props, masks) # [B, N, D], [B, N, N, D]
        
        local_enthalpy_embeds = self.enthalpy_mlp(edge_embed) # [B, N, N, D]
        enthalpy_gates = self.enthalpy_gate(edge_embed).sigmoid() * (core_masks[:,None,:,None] & core_masks[:,:,None,None]) # [B, N, N, 1]
        enthalpy_embed = local_enthalpy_embeds * enthalpy_gates # [B, N, N, D]
        
        entropy_embed = self.entropy_gate(node_embed).sigmoid() * core_masks[:,:,None] * self.entropy_mlp(node_embed) # [B, N, D]

        affinities = (self.enthalpy_head(enthalpy_embed).sum(dim=(1, 2)) + self.entropy_head(entropy_embed).sum(dim=1)).squeeze(1) # [B]
...
class AffinityLoss(nn.Module):
    ...
    def forward(self, affinities, affinity_targets, **kwargs):
        ...
        pred_diff = affinities[:,None] - affinities[None,:] # [B, B]
        label_diff = affinity_targets[:,None] - affinity_targets[None,:] # [B, B]
        mask = (label_diff > 0) # [B, B]
        rank_loss = (F.relu(-pred_diff) * mask).sum() / (mask.sum() + 1e-8) * self.rank_weight # []
