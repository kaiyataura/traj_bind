...
def evaluate(model, dataloader, loss_fn, device, dtype, chunk_size, desc=None, **kwargs):
    model.eval()
    total_loss = 0.0
    ...
    with tqdm(dataloader, desc=desc, leave=False, smoothing=0.0) as pbar:
        with torch.no_grad():
            for i, batch in enumerate(pbar):
                batch = {k:(x.to(device, dtype) if x.is_floating_point() else x.to(device)) if isinstance(x, torch.Tensor) else x for k, x in batch.items()}
                out_dict = model(**batch, mode='train', chunk_size=chunk_size)
                loss, loss_dict = loss_fn(**out_dict, **batch, **kwargs, mode='valid')
...
def train(data_dir: str, mode: Literal['pretrain', 'teacher', 'student'] = 'pretrain', 
          ...
          num_epochs: int = 100, batch_size: int = 32, accum_size: int = 1, chunk_size: int = 2, num_frames: int = 16, valid_frac: float = 0.1,
          lr_readout: float = 1e-3, lr_scaffold: float = 1e-3, stop_lr: float | None = 2e-8,
    ...
    if mode == 'pretrain':
        ...
        loss_fn = InteractionLoss(3.94492979, 1.4795234, 0.84148928, 0.52675139)
        ...
        model = TeacherModel(**kwargs).to(device).to(dtype)
        loss_fn = JointLoss([(InteractionLoss(3.94492979, 1.4795234, 0.84148928, 0.52675139), 1.0), (AffinityLoss(), 1.0)]).to(dtype)
        dataset = TrajectoryDataset(data_dir, num_frames=num_frames)
        ...
        for epoch in pbar1:
            ...
            with tqdm(train_dataloader, desc="Batches", leave=False, smoothing=0.0) as pbar2:
                ...
                for i, batch in enumerate(pbar2):
                    ...
                    out_dict = model(**batch, mode='train', chunk_size=chunk_size)
                    loss, loss_dict = loss_fn(**out_dict, **batch, mode='train')
            ...
            if eval_every and epoch % eval_every == 0:
                eval_loss, eval_dict = evaluate(model, valid_dataloader, loss_fn, device, dtype, chunk_size, desc="Validating")
...
if __name__ == '__main__':
    ...
    #     batch_size=8, chunk_size=2, num_frames=16, lr_readout=1e-3, lr_scaffold=5e-4,
    #     # profile_time=True,
    #     param_every=1, eval_every=1, checkpoint_every=1,
    #     out_dir='checkpoints/run25'
    # )
    train(
        data_dir='data/dynarepo/dataset.h5', mode='pretrain',
        device='mps', dtype=torch.float32, num_epochs=1000, valid_frac=0.1,
        batch_size=2, accum_size=16, lr_readout=1e-3, lr_scaffold=1e-3,
        param_every=1, eval_every=1, checkpoint_every=10,
        out_dir='checkpoints/run49_pretrain',
        num_layers=0
    )