...
class TeacherModel(nn.Module):
    ...
    def forward(self, coords: torch.Tensor, residues: torch.Tensor, props: torch.Tensor, masks: torch.Tensor, core_masks: torch.Tensor, mode: Literal['inference', 'train', 'embed'] = 'inference', chunk_size: int = 2, **kwargs):
        ...
        def _chunk_forward(coord_chunk, residue_chunk, prop_chunk, mask_chunk, core_mask_chunk):
            ...
            weight = self.gate_mlp(edge_embed_chunk).masked_fill(~mask, -1e9).view(C, -1).softmax(-1).view(C, N, N, 1) # [C, N, N, 1]
        ...
        node_embed = torch.zeros(B, N, D, device=coords.device, dtype=graph_embeds.dtype) # [B, N, D]
        edge_embed = torch.zeros(B, N, N, D, device=coords.device, dtype=graph_embeds.dtype) # [B, N, N, D]
...
class AffinityLoss(nn.Module):
    ...
    def forward(self, affinities, affinity_targets, **kwargs):
        ...
        if affinities.shape[0] > 1:
            ...
            mask = (label_diff > 0).to(affinities.dtype).unsqueeze(-1) # [B, B, 1]
