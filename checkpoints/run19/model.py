...
class SpatialEncoder(nn.Module):
    ...
    def forward(self, coords: torch.Tensor, residues: torch.Tensor, props: torch.Tensor, masks: torch.Tensor):
        ...
        for layer in self.attn_layers:
            if self.training: node_embed, edge_embed = torch.utils.checkpoint.checkpoint(layer, node_embed, edge_embed, dir, edge_mask, frame, use_reentrant=False) # [B, N, D], [B, N, N, D] # type: ignore
            else: node_embed, edge_embed = layer(node_embed, edge_embed, dir, edge_mask, frame) # [B, N, D], [B, N, N, D]
            
        return node_embed, edge_embed


class TeacherModel(nn.Module):
    def __init__(self, hidden_dim: int = 128, chunk_size: int = 2, **kwargs):
        super().__init__()
    ...
    def forward(self, coords: torch.Tensor, residues: torch.Tensor, props: torch.Tensor, masks: torch.Tensor, core_masks: torch.Tensor, inference: bool = False, **kwargs):
        ...
        for t in range(0, T, self.chunk_size):
            C = min(self.chunk_size, T - t)
            coord_chunk = coords[:, t:t+C].reshape(B * C, N, 4, 3) # [B * C, N, 4, 3]
            residue_chunk = residues.repeat_interleave(C, 0) # [B * C, N]
            prop_chunk = props.repeat_interleave(C, 0) # [B * C, N, 5]
            mask_chunk = masks.repeat_interleave(C, 0) # [B * C, N]
            
            if self.training: _, edge_embed_flat = checkpoint(self.spatial_encoder, coord_chunk, residue_chunk, prop_chunk, mask_chunk, use_reentrant=False) # [B * C, N, D], [B * C, N, N, D] # type: ignore
            else: _, edge_embed_flat = self.spatial_encoder(coord_chunk, residue_chunk, prop_chunk, mask_chunk) # [B * C, N, D], [B * C, N, N, D]
            edge_embeds = edge_embed_flat.view(B, C, N, N, -1) # [B, C, N, D], [B, C, N, N, D]
        
            mask = core_masks[:,None,None,:,None] & core_masks[:,None,:,None,None] # [B, 1, N, N, 1]
            weight = self.gate_mlp(edge_embeds).masked_fill(~mask, -1e9).view(B, C, -1).softmax(-1).view(B, C, N, N, 1) # [B, C, N, N, 1]
            graph_embeds = (weight * edge_embeds).sum((2,3)) # [B, C, D]
            all_graph_embeds.append(graph_embeds)

            interactions = self.interaction_head(edge_embeds) # [B, C, N, N, 4]
            all_interactions.append(interactions)

        graph_embeds = torch.concat(all_graph_embeds, dim=1) # [B, T, D]
        interactions = torch.concat(all_interactions, dim=1) # [B, T, N, N, 4]
        ...
        for t in range(0, T, self.chunk_size):
            C = min(self.chunk_size, T - t)
            coord_chunk = coords[:, t:t+C].view(B * C, N, 4, 3) # [B * C, N, 4, 3]
            residue_chunk = residues.repeat_interleave(C, 0) # [B * C, N]
            prop_chunk = props.repeat_interleave(C, 0) # [B * C, N, 5]
            mask_chunk = masks.repeat_interleave(C, 0) # [B * C, N]
            
            node_embed_flat, edge_embed_flat = self.spatial_encoder(coord_chunk, residue_chunk, prop_chunk, mask_chunk) # [B * C, N, D], [B * C, N, N, D]
            node_embed = node_embed + node_embed_flat.view(B, C, N, -1) * probs[:, t].view(B, 1, 1) # [B, N, D]
            edge_embed = edge_embed + edge_embed_flat.view(B, C, N, N, -1) * probs[:, t].view(B, 1, 1, 1) # [B, N, N, D]
