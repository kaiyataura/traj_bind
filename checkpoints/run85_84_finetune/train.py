...
def embed_dataset(data_file, embeds_file, model, num_frames, device, dtype, chunk_size):
    dataset = TrajectoryDataset(data_file, num_frames=num_frames)
    ...
    embeds: dict[str, list[torch.Tensor]] = {
        'enthalpy_embed_targets': [None] * len(dataset), # type: ignore
        'entropy_embed_targets': [None] * len(dataset) # type: ignore
    }
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Computing Teacher Embeddings", leave=False, smoothing=0.0):
            ...
            for i, idx in enumerate(batch['indices'].cpu().tolist()):
                n = len(batch['residues'])
                embeds['enthalpy_embed_targets'][idx] = enthalpy_embed[i,:n,:n].contiguous()
                embeds['entropy_embed_targets'][idx] = entropy_embed[i,:n].contiguous()
...
def train(data_file: str, mode: Literal['pretrain', 'teacher', 'distill', 'student'] = 'pretrain', 
    ...
    if mode == 'pretrain':
        ...
        dataset = TrajectoryDataset(data_file, num_frames=1)
        ...
        if base_ckpt is None: raise ValueError("Argument `base_ckpt` must be defined for distill mode.")
        ...
        model.enthalpy_head.load_state_dict(teacher.enthalpy_head.state_dict())
        model.entropy_head.load_state_dict(teacher.entropy_head.state_dict())
        model.affinity_bias.data.copy_(teacher.affinity_bias.data)
        model.enthalpy_head.requires_grad_(False)
        model.entropy_head.requires_grad_(False)
        model.affinity_bias.requires_grad_(False)

        embeds_file = os.path.join(os.path.dirname(base_ckpt), 'embeds.pt')
        if not os.path.exists(embeds_file): embed_dataset(data_file, embeds_file, teacher, num_frames, device, dtype, chunk_size)

        dataset = TrajectoryDataset(data_file, num_frames=1, embeds_file=embeds_file)

    elif mode == 'student':
        model = StudentModel(**kwargs).to(device).to(dtype)
        if base_ckpt is not None: load_checkpoint(base_ckpt, model=model)
        loss_fn = AffinityLoss().to(dtype)
        dataset = TrajectoryDataset(data_file, num_frames=1)
        
    else: raise ValueError("Argument `mode` must be 'pretrain', 'teacher', 'distill', or 'student'.")
    ...
    for name, p in model.named_parameters():
        ...
        if p.ndim < 2 or 'bias' in name or 'norm' in name or group == 'head': nodecay_params[group].append(p)
...
if __name__ == '__main__':
    ...
    # train(
    #     data_file='data/dynarepo/dataset.h5', mode='pretrain',
    #     device='mps', dtype=torch.float32, num_epochs=100, valid_frac=0.1,
    #     batch_size=8, accum_size=1, chunk_size=1, num_frames=1, 
    #     lr_head=1e-3, lr_gate=1e-3, lr_scaffold=1e-3, warmup_epochs=2, patience=10,
    #     param_every=1, eval_every=1, checkpoint_every=5,
    #     out_dir='checkpoints/run73_pretrain',
    # )

    # train(
    #     data_file='data/dynarepo/dataset.h5', mode='teacher',
    #     device='mps', dtype=torch.float32, num_epochs=200, valid_frac=0.1,
    #     batch_size=8, accum_size=1, chunk_size=1, num_frames=4, 
    #     lr_head=1e-3, lr_gate=1e-3, lr_scaffold=1e-3, warmup_epochs=5, patience=10, weight_decay=1e-3,
    #     param_every=1, eval_every=1, checkpoint_every=5,
    #     base_ckpt='checkpoints/run81_73/epoch_0.pt',
    #     out_dir='checkpoints/run81_73',
    # )

    # train(
    #     data_file='data/dynarepo/dataset.h5', mode='teacher',
    #     device='mps', dtype=torch.float32, num_epochs=100, valid_frac=0.1,
    #     batch_size=8, accum_size=1, chunk_size=1, num_frames=16, 
    #     lr_head=1e-3, lr_gate=1e-4, lr_scaffold=1e-4, warmup_epochs=2, patience=10, weight_decay=1e-3,
    #     param_every=1, eval_every=1, checkpoint_every=1,
    #     base_ckpt='checkpoints/run81_73/epoch_130.pt',
    #     out_dir='checkpoints/run82_81',
    # )
    
    # train(
    #     data_file='data/dynarepo/dataset.h5', mode='distill',
    #     device='mps', dtype=torch.float32, num_epochs=100, valid_frac=0.1,
    #     batch_size=8, accum_size=1, chunk_size=1, num_frames=16, 
    #     lr_head=1e-3, lr_gate=5e-4, lr_scaffold=1e-4, warmup_epochs=2, patience=10, weight_decay=1e-3,
    #     param_every=1, eval_every=1, checkpoint_every=1,
    #     base_ckpt='checkpoints/run82_81/best.pt',
    #     out_dir='checkpoints/run84_82_student'
    # )

    train(
        data_file='data/pdbbind/dataset.h5', mode='student',
        device='mps', dtype=torch.float32, num_epochs=100, valid_frac=0.1,
        batch_size=8, accum_size=1, chunk_size=1, num_frames=1, 
        lr_head=5e-4, lr_gate=2e-4, lr_scaffold=1e-5, warmup_epochs=2, patience=10, weight_decay=1e-3,
        param_every=1, eval_every=1, checkpoint_every=1,
        base_ckpt='checkpoints/run84_82_student/epoch_85.pt',
        out_dir='checkpoints/run85_84_finetune'
    )