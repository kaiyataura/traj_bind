...
class DistanceEmbedding(nn.Module):
    ...
    def forward(self, coords: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        env = env * (dists < self.r_max).bfloat16()
...
class TeacherModel(nn.Module):
    ...
    def forward(self, coords: torch.Tensor, residues: torch.Tensor, props: torch.Tensor, masks: torch.Tensor, core_masks: torch.Tensor, mode: Literal['inference', 'train', 'embed'] = 'inference', **kwargs):
        # coords: [B, T, N, 4, 3]
        # residues: [B, N]
        # props: [B, N, 5]
        # masks: [B, N]
        ...
        node_embed = torch.zeros(B, N, D, device=coords.device, dtype=torch.bfloat16) # [B, N, D]
        edge_embed = torch.zeros(B, N, N, D, device=coords.device, dtype=torch.bfloat16) # [B, N, N, D]
...
class AffinityLoss(nn.Module):
    ...
    def forward(self, affinities, affinity_targets, **kwargs):
        ...
        if affinities.shape[0] > 1:
            ...
            mask = (label_diff > 0).bfloat16().unsqueeze(-1) # [B, B, 1]
