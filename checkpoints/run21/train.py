...
def embed_dataset(data_dir, embeds_file, model, num_frames, device):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Computing Teacher Embeddings", leave=False, smoothing=0.0):
            batch = {k:(x.to(device, dtype=torch.bfloat16) if x.is_floating_point() else x.to(device)) if isinstance(x, torch.Tensor) else x for k, x in batch.items()}
            out_dict = model(**batch, mode='embed')
    ...
    with tqdm(dataloader, desc=desc, leave=False, smoothing=0.0) as pbar:
        with torch.no_grad():
            for i, batch in enumerate(pbar):
                batch = {k:(x.to(device, dtype=torch.bfloat16) if x.is_floating_point() else x.to(device)) if isinstance(x, torch.Tensor) else x for k, x in batch.items()}
                out_dict = model(**batch, mode='train')
...
def train(data_dir: str, mode: Literal['teacher', 'student'] = 'teacher', 
    ...
    if mode == 'teacher':
        ...
        model = TeacherModel().to(device).to(torch.bfloat16)
        loss_fn = JointLoss([(InteractionLoss(0.001061, 6.777358, 17.148979, 11670.543898), 1.0), (AffinityLoss(), 1.0)]).to(torch.bfloat16)
        dataset = TrajectoryDataset(data_dir, num_frames=num_frames)
        
    elif mode == 'student':
        if base_ckpt is None: raise ValueError("Argument `base_ckpt` must be defined for student mode.")
        model = StudentModel().to(device).to(torch.bfloat16)
        loss_fn = JointLoss([(DistillationLoss(), 1.0), (AffinityLoss(), 1.0)]).to(torch.bfloat16)
        
        teacher = TeacherModel().to(device).to(torch.bfloat16)
