...
class DistanceEmbedding(nn.Module):
    ...
    def __init__(self, dist_dim: int = 32, r_min: float = 1.0, r_max: float = 15.0):
        super().__init__()
    ...
    def forward(self, coords: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        c1 = coords.unsqueeze(2).unsqueeze(4) # [B, N, 1, 4, 1, 3]
        c2 = coords.unsqueeze(1).unsqueeze(3) # [B, 1, N, 1, 4, 3]
        
        all_dists = torch.sqrt(((c1 - c2) ** 2).sum(dim=-1) + 1e-8) # [B, N, N, 4, 4]
        all_dists = all_dists.view(B, N, N, 16)
        
        rbfs = torch.exp(((all_dists.unsqueeze(-1) - self.centers) ** 2) * -self.gamma) # [B, N, N, 16, Sd]
        rbfs = rbfs.flatten(-2) # [B, N, N, 16 * Sd]
        
        min_dists = all_dists.min(dim=-1)[0]
        return rbfs, min_dists, all_dists # [B, N, N, 16 * Sd], [B, N, N], [B, N, N, 16]
...
class AngleEmbedding(nn.Module):
    ...
    def __init__(self, angle_dim: int = 16):
        super().__init__()
        centers = torch.linspace(-1, 1, angle_dim)
        self.register_buffer('centers', centers)
        self.gamma: float = -4.0 * math.log(0.5) / ((centers[1].item() - centers[0].item()) ** 2)
        
    def forward(self, coords: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        B, N, _, _ = coords.shape
        ca, c, n, sc = coords.unbind(2)
        
        v1 = F.normalize(c - ca, dim=-1)
        v2 = F.normalize(n - ca, dim=-1)
        v3 = F.normalize(sc - ca, dim=-1)
        v4 = F.normalize(torch.cross(v1, v2, dim=-1), dim=-1)
        
        diff = ca[:, None, :, :] - ca[:, :, None, :]
        dir = diff / (diff.norm(dim=-1, keepdim=True) + 1e-8)
        
        frame = torch.stack([v1, v4, torch.cross(v1, v4, dim=-1)], dim=-1) # [B, N, 3, 3]
        
        angs = torch.stack([
            (v1.unsqueeze(2) * dir).sum(dim=-1),
            (v2.unsqueeze(2) * dir).sum(dim=-1),
            (v3.unsqueeze(2) * dir).sum(dim=-1),
            (v1.unsqueeze(1) * dir).sum(dim=-1),
            (v2.unsqueeze(1) * dir).sum(dim=-1),
            (v3.unsqueeze(1) * dir).sum(dim=-1),
            (v1.unsqueeze(2) * v1.unsqueeze(1)).sum(dim=-1),
            (v2.unsqueeze(2) * v2.unsqueeze(1)).sum(dim=-1),
            (v3.unsqueeze(2) * v3.unsqueeze(1)).sum(dim=-1),
            (v4.unsqueeze(2) * v4.unsqueeze(1)).sum(dim=-1),
        ], dim=-1) # [B, N, N, 10]
        
        rbfs = torch.exp(((angs.unsqueeze(-1) - self.centers) ** 2) * -self.gamma).flatten(-2) # [B, N, N, Sa]
        return rbfs, dir, frame, angs
...
class SpatialEncoder(nn.Module):
    def __init__(self, hidden_dim: int = 128, res_dim: int = 16, dist_dim: int = 16, angle_dim: int = 8, num_layers: int = 2, num_heads: int = 8, r_max: float = 15.0, dropout: float = 0.1):
        ...
        geom_dim = dist_dim * 16 + 10 * angle_dim
        ...
        self.edge_proj = nn.Sequential(
            nn.Linear(geom_dim + 2 * hidden_dim, hidden_dim * 2),
            nn.SiLU(),
            nn.Linear(hidden_dim * 2, hidden_dim),
            nn.LayerNorm(hidden_dim)
        )
        ...
        self.attn_layers = nn.ModuleList([
            SpatialAttention(hidden_dim, geom_dim, num_heads=num_heads, dropout=dropout) for _ in range(num_layers)
        ])
    ...
    def forward(self, coords: torch.Tensor, residues: torch.Tensor, props: torch.Tensor, masks: torch.Tensor):
        ...
        dist_embed, dist, _ = self.dist_embedding(coords) # [B, N, N, Sd], [B, N, N], [B, N, N, 16]
        angle_embed, dir, frame, _ = self.angle_embedding(coords) # [B, N, N, Sa], [B, N, N, 3], [B, N, 3, 3], [B, N, N, 10]
        geom_embed = torch.concat([dist_embed, angle_embed], dim=-1) # [B, N, N, geom_dim]
...
class TeacherModel(nn.Module):
    ...
    def forward(self, coords: torch.Tensor, residues: torch.Tensor, props: torch.Tensor, masks: torch.Tensor, core_masks: torch.Tensor, mode: Literal['inference', 'pretrain', 'train', 'embed'] = 'inference', chunk_size: int = 2, **kwargs):
        ...
        _, edge_embed_flat = self.spatial_encoder(coords_flat, residues_flat, props_flat, masks_flat) # [B * T, N, D], [B * T, N, N, D]

        interactions = self.interaction_head(edge_embed_flat).view(B, T, N, N, -1)
