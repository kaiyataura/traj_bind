...
if __name__ == '__main__':
    ...
    train(
        data_dir='data/dynarepo/dataset.h5', mode='teacher',
        device='mps', dtype=torch.float32, num_epochs=1000, valid_frac=0.1,
        batch_size=8, accum_size=1, chunk_size=1, num_frames=16, lr_head=1e-3, lr_gate=5e-4, lr_scaffold=1e-4, warmup_epochs=0, patience=20, weight_decay=1e-2,
        param_every=1, eval_every=1, checkpoint_every=1,
        base_ckpt='checkpoints/run78_76_full/epoch_0.pt',
        out_dir='checkpoints/run78_76_full',
    )