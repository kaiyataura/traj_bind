...
class DistanceEmbedding(nn.Module):
    ...
    def forward(self, coords: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        env = env * (dists < self.r_max).to(env.dtype)
...
class SpatialAttention(nn.Module):
    def __init__(self, hidden_dim: int, num_heads: int = 8, dropout: float = 0.1):
        ...
        self.edge_proj_src = nn.Linear(hidden_dim, hidden_dim, bias=False)
        self.edge_proj_dst = nn.Linear(hidden_dim, hidden_dim, bias=False)
        self.edge_proj_bias = nn.Parameter(torch.zeros(hidden_dim))
        nn.init.zeros_(self.edge_proj_src.weight)
        nn.init.zeros_(self.edge_proj_dst.weight)
        nn.init.zeros_(self.edge_proj_bias)
    ...
    def forward(self, node_embed: torch.Tensor, edge_embed: torch.Tensor, dir: torch.Tensor, mask: torch.Tensor, frame: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        edge_embed = edge_embed + self.edge_proj_src(node_embed).unsqueeze(2) + self.edge_proj_dst(node_embed).unsqueeze(1) + self.edge_proj_bias # [B, N, N, D]
...
class TeacherModel(nn.Module):
    ...
    def forward(self, coords: torch.Tensor, residues: torch.Tensor, props: torch.Tensor, masks: torch.Tensor, core_masks: torch.Tensor, mode: Literal['inference', 'train', 'embed'] = 'inference', chunk_size: int = 2, **kwargs):
        # coords: [B, T, N, 4, 3]
        # residues: [B, N]
        # props: [B, N, 5]
        # masks: [B, N]
        ...
        def _ckpt_forward(coord_chunk, residue_chunk, prop_chunk, mask_chunk, core_mask_chunk):
            ...
            edge_weight = self.edge_gate(edge_embed_chunk).masked_fill(~edge_mask, -1e9).view(C, -1).softmax(-1).view(C, N, N, 1) # [C, N, N, 1]
            edge_graph_embeds = (edge_weight * edge_embed_chunk).sum((1,2)) # [C, D]
        ...
        for i in range(0, B * T, chunk_size):
            ...
            node_weight = self.node_gate(node_embed_chunk).masked_fill(~node_mask, -1e9).view(C, -1).softmax(-1).view(C, N, 1) # [C, N, 1]
            node_graph_embeds = (node_weight * node_embed_chunk).sum(1) # [C, D]
...
class StudentModel(nn.Module):
    ...
    def forward(self, coords: torch.Tensor, residues: torch.Tensor, props: torch.Tensor, masks: torch.Tensor, core_masks: torch.Tensor, mode: Literal['inference', 'train'] = 'inference', **kwargs):
        ...
        edge_weight = self.edge_gate(edge_embed).masked_fill(~edge_mask, -1e9).view(B, -1).softmax(-1).view(B, N, N, 1) # [B, N, N, 1]
        enthalpy_embed = self.enthalpy_mlp((edge_weight * edge_embed).sum((1,2))) # [B, D]
        
        node_mask = core_masks[:,:,None] # [B, N, 1]
        node_weight = self.node_gate(node_embed).masked_fill(~node_mask, -1e9).view(B, -1).softmax(-1).view(B, N, 1) # [B, N, 1]
        entropy_embed = self.entropy_mlp((node_weight * node_embed).sum(1)) # [B, D]
