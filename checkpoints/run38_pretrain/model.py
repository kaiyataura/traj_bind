...
class DistanceEmbedding(nn.Module):
    ...
    def __init__(self, dist_dim: int = 32, r_min: float = 0.5, r_max: float = 15.0):
        super().__init__()
    ...
    def forward(self, coords: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        return rbfs, dists # [B, N, N, Sd], [B, N, N]
...
class SpatialEncoder(nn.Module):
    def __init__(self, hidden_dim: int = 128, res_dim: int = 16, dist_dim: int = 16, angle_dim: int = 8, num_layers: int = 2, num_heads: int = 8, r_max: float = 15.0, dropout: float = 0.1):
        ...
        self.edge_proj = nn.Sequential(
            nn.Linear(dist_dim + 10 * angle_dim + 2 * hidden_dim, hidden_dim),
            nn.LayerNorm(hidden_dim)
        )
...
class InteractionLoss(nn.Module):
    ...
    def forward(self, interactions, interaction_targets, coords, core_masks, mode='train', **kwargs):
        ...
        if mode == 'eval': losses = F.huber_loss(interactions / stds, interaction_targets / stds, reduction='none', delta=1.0) # [B, T, N, N, 4]
        else: losses = F.mse_loss(interactions / stds, interaction_targets / stds, reduction='none') # [B, T, N, N, 4]
