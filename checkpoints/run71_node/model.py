...
class TeacherModel(nn.Module):
    def __init__(self, hidden_dim: int = 128, num_channels: int = 8, **kwargs):
        super().__init__()
        ...
        nn.init.normal_(self.enthalpy_gate[-1].weight, std=0.01)
        ...
        nn.init.normal_(self.entropy_gate[-1].weight, std=0.01)
        ...
        self.enthalpy_head = nn.Linear(hidden_dim, num_channels, bias=False)
        nn.init.normal_(self.enthalpy_head.weight, std=0.01)
        
        self.entropy_head = nn.Linear(hidden_dim, num_channels, bias=False)
        nn.init.normal_(self.entropy_head.weight, std=0.01)
    ...
    def forward(self, coords: torch.Tensor, residues: torch.Tensor, props: torch.Tensor, valid_masks: torch.Tensor, core_masks: torch.Tensor, ligand_masks: torch.Tensor, mode: Literal['inference', 'pretrain', 'train', 'embed'] = 'inference', chunk_size: int = 1, **kwargs):
        ...
        valid_masks_flat = valid_masks.repeat_interleave(T, 0) # [B * T, N]

        def process_chunk(coords_chunk, residues_chunk, props_chunk, valid_masks_chunk):
            node_embeds_chunk, edge_embeds_chunk, geom_embeds_chunk = self.spatial_encoder(coords_chunk, residues_chunk, props_chunk, valid_masks_chunk) # [C, N, D], [C, N, N, D], [C, N, N, D]
            interactions_chunk = self.interaction_head(torch.concat([edge_embeds_chunk, geom_embeds_chunk], dim=-1)) if mode == 'pretrain' or mode == 'train' else None # [C, N, N, 4]
            return node_embeds_chunk, interactions_chunk

        node_embeds, interactions = [], []
        for start in range(0, B * T, chunk_size):
            end = min(start + chunk_size, B * T)
            coords_chunk = coords_flat[start:end] # [C, N, 4, 3]
            residues_chunk = residues_flat[start:end] # [C, N]
            props_chunk = props_flat[start:end] # [C, N, 5]
            valid_masks_chunk = valid_masks_flat[start:end] # [C, N]

            if self.training: node_embeds_chunk, interactions_chunk = checkpoint(process_chunk, coords_chunk, residues_chunk, props_chunk, valid_masks_chunk, use_reentrant=False) # type: ignore
            else: node_embeds_chunk, interactions_chunk = process_chunk(coords_chunk, residues_chunk, props_chunk, valid_masks_chunk)

            node_embeds.append(node_embeds_chunk)
            interactions.append(interactions_chunk)

        node_embeds = torch.concat(node_embeds).view(B, T, N, -1) # [B, T, N, D]
        interactions = torch.concat(interactions).view(B, T, N, N, 4) if mode == 'pretrain' or mode == 'train' else None # [B, T, N, N, 4]

        if mode == 'pretrain': return {'interactions': interactions}
        
        enthalpy_embeds  = self.enthalpy_mlp(node_embeds) # [B, T, N, D]
        enthalpies = (self.enthalpy_head(enthalpy_embeds) * self.enthalpy_gate(node_embeds).sigmoid() * core_masks[:,:,None]).sum((-1,-2)) # [B, T]
        probs = F.softmax(torch.exp(self.log_beta) * enthalpies, dim=-1) # [B, T]
        enthalpy = (enthalpies * probs).sum(dim=1) # [B]
        
        entropy_embeds = self.entropy_mlp(node_embeds) # [B, T, N, D]
        entropy_embed = ((probs[:,:,None,None,None] * probs[:,None,:,None,None]) * torch.abs(entropy_embeds[:,:,None,:,:] - entropy_embeds[:,None,:,:,:])).sum((-3,-4)) # [B, N, D]
        node_embed = (node_embeds * probs[:,:,None,None]).sum(-3) # [B, N, D]
        entropy = (self.entropy_head(entropy_embed) * self.entropy_gate(node_embed).sigmoid() * core_masks[:,:,None]).sum((-1,-2)) # [B]
        ...
        enthalpy_embed = (enthalpy_embeds * probs[:,:,None,None]).sum(-3) # [B, N, D]
        return {'node_embed': node_embed, 'enthalpy_embed': enthalpy_embed, 'entropy_embed': entropy_embed}


class StudentModel(nn.Module):
    def __init__(self, hidden_dim: int = 128, num_channels: int = 8, noise: float = 0.01, **kwargs):
        super().__init__()
        self.noise = noise
        self.spatial_encoder = SpatialEncoder(hidden_dim=hidden_dim, **kwargs)
    ...
    def forward(self, coords: torch.Tensor, residues: torch.Tensor, props: torch.Tensor, valid_masks: torch.Tensor, core_masks: torch.Tensor, ligand_masks: torch.Tensor, mode: Literal['inference', 'train'] = 'inference', **kwargs):
        ...
        node_embed, edge_embed, geom_embed = self.spatial_encoder(coords, residues, props, valid_masks) # [B, N, D], [B, N, N, D], [B, N, N, D]
        
        enthalpy_embed = self.enthalpy_mlp(node_embed) # [B, N, D]
        entropy_embed = self.entropy_mlp(node_embed) # [B, N, D]
        
        enthalpies = (self.enthalpy_head(enthalpy_embed) * self.enthalpy_gate(node_embed).sigmoid() * core_masks[:,:,None]).sum(dim=(1, 2)) # [B]
