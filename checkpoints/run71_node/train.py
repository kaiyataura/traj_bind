        ...
        for epoch in pbar1:
            ...
            if eval_every and epoch % eval_every == 0:
                ...
                res_str = f"Epoch {epoch} - " \
                          ...
                          f"LR (scaffold): {optimizer.param_groups[-1]['lr']:.1e}"
...
if __name__ == '__main__':
    ...
    train(
        data_dir='data/dynarepo/dataset.h5', mode='teacher',
        device='mps', dtype=torch.float32, num_epochs=1000, valid_frac=0.1,
        batch_size=8, accum_size=1, chunk_size=1, num_frames=16, lr_head=1e-4, lr_gate=5e-3, lr_scaffold=1e-3, warmup_epochs=2, patience=10,
        param_every=1, eval_every=1, checkpoint_every=5,
        out_dir='checkpoints/run71_node',
    )