...
def train(data_dir: str, mode: Literal['pretrain', 'teacher', 'student'] = 'pretrain', 
          ...
          num_epochs: int = 100, batch_size: int = 32, chunk_size: int = 2, num_frames: int = 16, valid_frac: float = 0.1,
    ...
    if mode == 'pretrain':
        ...
        model = TeacherModel().to(device).to(dtype)
        loss_fn = InteractionLoss(115.49647766, 1.42579313, 0.81054552, 0.52631601)
        ...
        model = TeacherModel().to(device).to(dtype)
        loss_fn = JointLoss([(InteractionLoss(115.49647766, 1.42579313, 0.81054552, 0.52631601), 1.0), (AffinityLoss(), 1.0)]).to(dtype)
        dataset = TrajectoryDataset(data_dir, num_frames=num_frames)
        
    elif mode == 'student':
        if base_ckpt is None: raise ValueError("Argument `base_ckpt` must be defined for student mode.")
        model = StudentModel().to(device).to(dtype)
        loss_fn = JointLoss([(DistillationLoss(), 1.0), (AffinityLoss(), 1.0)]).to(dtype)
        
        teacher = TeacherModel().to(device).to(dtype)
...
if __name__ == '__main__':
    ...
    train(
        ...
        device='mps', dtype=torch.float32, num_epochs=1000, valid_frac=0.1,
        batch_size=4, lr_readout=1e-3, lr_scaffold=5e-4,
        param_every=1, eval_every=1, checkpoint_every=10,
        out_dir='checkpoints/run28_pretrain'
    )